...
def init_blocks():
    ...
    with gr.Blocks(title=title, delete_cache=(3600, 3660)) as demo:
        ...
        with gr.Row():
            ...
            # --- Add explicit API endpoint for animate_from_url ---
            with gr.Row(visible=False): # Hidden row
                api_animate_from_url_trigger_btn = gr.Button("API Trigger for Animate From URL")
            # Re-use the dummy Textbox output for the API to ensure string passthrough
            api_animate_from_url_trigger_btn.click(
                fn=animate_from_url, 
                inputs=[url_input], # The `animate_from_url` function takes `model_url` (string)
                outputs=[api_dummy_string_output], # Output to a dummy Textbox ensures string (persistent_url) return
                api_name="exposed_animate_from_url_api"  # New explicit API name for animation
            )
            # --- End explicit API endpoint section for animation ---
...
def animate_from_url(model_url):
    ...
    if not os.path.isfile(animation_file):
        # Attempt fallback relative path if needed (adjust based on actual structure)
        animation_file = os.path.abspath(os.path.join(os.path.dirname(__file__), 'data/Standard Run.fbx')) 
        if not os.path.isfile(animation_file):
             raise gr.Error(f"Default animation file 'Standard Run.fbx' not found at expected paths.")
    db = DB()
    for step in _pipeline(
        input_path=local_path,
        is_gs=False,
        ...
        pass
    # Always return the GLB preview model path generated by vis_blender
    if db.anim_vis_path and os.path.isfile(db.anim_vis_path):
        animated_path = db.anim_vis_path
    else:
        # Fallback or error if GLB preview wasn't created
        if db.anim_path and os.path.isfile(db.anim_path):
            print(f"Warning: Animated GLB preview (anim_vis_path) not found. Using source animation path: {db.anim_path}")
            animated_path = db.anim_path # This might be FBX or BLEND, upload might behave differently
        else:
            raise gr.Error("Animation failed: No output file (GLB or source) found")

    # Upload to Render.com
    with open(animated_path, "rb") as f:
        # Determine MIME type (prefer gltf-binary if it's a GLB)
        mime_type = "model/gltf-binary" if animated_path.lower().endswith(".glb") else "application/octet-stream"
        files = {"modelFile": (os.path.basename(animated_path), f, mime_type)}
        # Create a descriptive payload
        original_basename = os.path.splitext(os.path.basename(model_url))[0]
        upload_payload = {
            "clientType": "playcanvas",
            "modelStage": "mia_animated_run", # Specific stage for this animation
            "baseName": original_basename,
            "prompt": f"animated model from {original_basename}" # Optional descriptive prompt
        }
        response = requests.post(
            "https://viverse-backend.onrender.com/api/upload-rigged-model",
            files=files,
            data=upload_payload # Use descriptive payload
        )
        if response.status_code != 200:
            raise gr.Error(f"Upload of animated model to Render.com failed: {response.text}")
        result = response.json()
        persistent_url = result.get("persistentUrl")
        if not persistent_url:
            raise gr.Error("No persistent URL returned from Render.com for animated model")
        print(f"Uploaded animated model to: {persistent_url}")

    return persistent_url # MODIFIED: Return the persistent URL from the server
